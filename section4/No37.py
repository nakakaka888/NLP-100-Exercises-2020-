...
for dicts in result_lists:
    ...
    if flag:
        for dict in dicts:
            if dict['surface'] not in coocat_dict:
                coocat_dict[dict['surface']] = 1
            else:
                coocat_dict[dict['surface']] += 1
        flag = False

coocat_dict.pop('猫')
coocat_dict = sorted(coocat_dict.items(), reverse=True, key = lambda x : x[1])

#グラフの描画
keys = []
values = []
for i in range(10):
    keys.append(coocat_dict[i][0])
    values.append(coocat_dict[i][1])

plt.xlabel("共起単語")
plt.ylabel("共起回数")
plt.bar(keys, values)
plt.savefig('./section4/No37.png')
